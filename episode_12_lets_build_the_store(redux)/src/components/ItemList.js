import { useDispatch } from "react-redux";
import { addItem } from "../utils/cartSlice";
import { CDN_URL } from "../utils/constants";
const ItemList = ({ items }) => {
  console.log(items);
  
  const dispatch = useDispatch();  //we have useslecttor for reading and for dispatching we have useDispatch.
  
  const handleAddItem = (item)=>{
    //on click we have to dispacth action from cartSlice i,e addItem for that wwe are using dispatch function
      dispatch(addItem(item));//what ever add inside this addItem will go inside cart.(action.payload)
      alert('Item added to cart succesfully');
    };


  return (
    <div>
      {items.map((item) => (
        <div
          key={item.card.info.id}
          className="p-2 m-2 border-gray-200 border-b-2 text-left flex justify-between"
        >
          <div className="w-9/12">
            <div className="py-2">
              <span>{item.card.info.name}</span>
              <span>
                - ₹
                {item.card.info.price / 100 ||
                  item.card.info.defaultPrice / 100}
              </span>
            </div>
            <p className="text-xs">{item.card.info.description}</p>
          </div>
          <div className="w-3/12">
            <div className="absolute">
              <button className="p-2 bg-black text-white shadow-lg text-sm rounded " onClick={()=>handleAddItem(item)}>Add +</button>
            </div>
            <img src={CDN_URL + item.card.info.imageId} className="rounded-lg"></img>
          </div>
        </div>
      ))}
    </div>
  );
};

export default ItemList;
