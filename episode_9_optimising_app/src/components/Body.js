...
const Body =()=>{
   ...
   //state -> super-powerful variable.
    ...
    const fetchData = async ()=>{
      ...
      if(onlineStatus === false) return <h1>Looks like your are offline pls check your internet</h1>
    ...
    return filteredRestaurant.length === 0 ? <Shimmer/> : (
      <div className='body'>
       ...
       <div className='rest-container'>
         {
          filteredRestaurant.map(restaurant=><Link key={restaurant.info.id}  to={'/restaurant/'+restaurant.info.id}><ReastaurantCard  resData={restaurant}/></Link>)
         }
       </div>
      </div>
    );
